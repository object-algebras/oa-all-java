...
public final class MumAlgGen implements MumAlg<MumAlgGen.TypeE, MumAlgGen.TypeP> {
    ...
    public TypeE lambdaNode(List<TypeE> p0, List<TypeE> p1) {
        return new TypeE() {
            public <E, P> E accept(MumAlg<E, P> alg) {
                return alg.lambdaNode(p0.stream().map((pTypeE) -> pTypeE.accept(alg)).collect(Collectors.toList()), p1
                        .stream().map((pTypeE) -> pTypeE.accept(alg)).collect(Collectors.toList()));
            }
        };
    }
